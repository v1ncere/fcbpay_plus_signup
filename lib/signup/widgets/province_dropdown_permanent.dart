...
class ProvinceDropdownPermanent extends StatelessWidget {
  const ProvinceDropdownPermanent({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<SignupBloc, SignupState>(
      buildWhen: (previous, current) => previous.provinceList != current.provinceList
      || previous.provincePermanent != current.provincePermanent
      ...
      builder: (context, state) {
        ...
        return DropdownButtonFormField(
          value: state.provincePermanent.value,
          hint: Text("Select Province"),
          decoration: InputDecoration(
            labelText: 'Province',
            labelStyle: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w700),
            errorText: state.provincePermanent.displayError?.text(),
          ...
          onChanged: (value) => context.read<SignupBloc>().add(ProvincePermanentChanged(value!)),
          items: state.provinceList.map((e) {
            return DropdownMenuItem<String>(
              ...
              onTap: () {
                ...
                ..add(AddressPermanentRefreshed())
                ..add(CityMunicipalityPermanentFetched(e.code));
              }
            );
          }).toList()
        );
