...
class CityMunicipalityDropdown extends StatelessWidget {
  const CityMunicipalityDropdown({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<SignupBloc, SignupState>(
      ...
      || previous.cityMunicipalityStatus != current.cityMunicipalityStatus,
      builder: (context, state) {
        if (state.cityMunicipalityStatus.isLoading) {
          return const Padding(
            padding: EdgeInsets.only(top: 5.0),
            child: ShimmerRectLoading()
          );
        }
        if (state.cityMunicipalityStatus.isFailure) {
          return Center(child: Text(state.message));
        }
        // default display
        return DropdownButtonFormField(
          value: state.cityMunicipality.value,
          decoration: InputDecoration(
            labelText: 'City/Municipality',
            labelStyle: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w700),
            errorText: state.cityMunicipality.displayError?.text(),
          ...
          items: state.cityMunicipalityList.map((e) {
            return DropdownMenuItem<String>(
              ...
              onTap: () { 
                ...
                ..add(PartialAddressRefreshed())
                ..add(BarangayFetched(e.code));
              },
            );
          }).toList()
        );
